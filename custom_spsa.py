...
class SPSA(Optimizer):
    ...
    def optimize(self, num_vars, objective_function, gradient_function=None, variable_bounds=None, initial_point=None, vector_gen=None):
    ...
    def _optimization(self, obj_fun, initial_theta, max_trials, save_steps=1, last_avg=1, vector_gen=None):
        ...
        for k in range(max_trials):
            ...
            #This is where my function deviates from the standard qiskit implementation
            #|XXXXXXXXXXXXXXXXXXXX|
            if vector_gen == None:
                delta = 2 * np.random.randint(2, size=np.shape(initial_theta)[0]) - 1
            else:
                delta = vector_gen(theta)
            #|XXXXXXXXXXXXXXXXXXXX|
       ...
       # print("THETA" + str(theta_best.shape))
